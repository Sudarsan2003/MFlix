...
const SpecificationShows = () => {
  ...
  useEffect(() => {
    axios.get(`https://api.themoviedb.org/3/tv/${id}?api_key=4a3be2203ea40ced49f144e93997c7aa`)
      .then((res) => {
        setShows(res.data);
      });
  }, [id]);
  ...
  useEffect(() => {
    axios.get(`https://api.themoviedb.org/3/tv/${id}/videos?api_key=4a3be2203ea40ced49f144e93997c7aa`)
      .then((res) => {
  ...
  useEffect(() => {
    axios.get(`https://api.themoviedb.org/3/tv/${id}/images?api_key=4a3be2203ea40ced49f144e93997c7aa`)
      .then((res) => {
  ...
  const handleWatchlistToggle = () => {
    if (isInWatchlist) {
      dispatch(removeFromWatchlist(shows.id));
    } else {
      dispatch(addToWatchlist(shows));
    }
  };

  if (!shows) return <div className="text-white text-center text-xl">Loading...</div>;
    ...
    <div className="text-white p-5 md:p-10">
      <div className="flex flex-col md:flex-row gap-6 md:gap-10">
        ...
        <div className="flex flex-col">
          <h1 className="text-2xl md:text-4xl font-bold">{shows.name}</h1>
          <p className="mt-3 text-sm md:text-lg text-gray-300">{shows.overview}</p>
          <p className="mt-3"><strong>First Air Date:</strong> {shows.first_air_date}</p>
          <p><strong>Rating:</strong> {shows.vote_average}/10</p>
          <p><strong>Seasons:</strong> {shows.number_of_seasons}</p>
          <p><strong>Episodes:</strong> {shows.number_of_episodes}</p>
...
export default SpecificationShows;
